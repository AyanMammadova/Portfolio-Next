...
const FlowerCanvas: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const renderTargets = [
      new THREE.WebGLRenderTarget(window.innerWidth, containerHeight),
      new THREE.WebGLRenderTarget(window.innerWidth, containerHeight),
    ];
    ...
    const fragmentShader = `
      ...
      void main() {
       ...
       vec3 flower_color = vec3(.5 + .2 * u_stop_randomizer[1], .05, .05);
    ...
    const createPlane = () => {
      shaderMaterial = new THREE.ShaderMaterial({
        uniforms: {
          ...
          u_ratio: { value: window.innerWidth / containerHeight },
    ...
    const updateSize = () => {
      shaderMaterial.uniforms.u_ratio.value = window.innerWidth / containerHeight;
      renderer.setSize(window.innerWidth, containerHeight);
      renderTargets[0].setSize(window.innerWidth, containerHeight);
      renderTargets[1].setSize(window.innerWidth, containerHeight);
    };
    ...
    const handleClick = (e: MouseEvent) => {
      if (!isTouchScreen) {
        ...
        pointer.y = (e.clientY - rect.top) / containerHeight;
        pointer.clicked = true;
      }
    };
    ...
    const handleTouchStart = (e: TouchEvent) => {
      ...
      const rect = container.getBoundingClientRect();
      pointer.x = (e.targetTouches[0].clientX - rect.left) / rect.width;
      pointer.y = (e.targetTouches[0].clientY - rect.top) / containerHeight;
      pointer.clicked = true;
    };
  ...
  const handleCleanClick = () => {
    const event = new CustomEvent('cleanCanvas');
    window.dispatchEvent(event);
  };

  return (
    <div ref={containerRef} className="relative w-full h-[100vh] overflow-hidden bg-black">
      <canvas ref={canvasRef} className="absolute top-0 left-0 block w-full h-[100vh]" />
   

      <div className="absolute bottom-0 left-0 w-full flex flex-col-reverse items-start">
        <button
          onClick={handleCleanClick}
          className="z-10 font-sans text-xs text-white select-none pb-2 pl-4 cursor-pointer underline opacity-50 hover:opacity-100 transition-opacity"
